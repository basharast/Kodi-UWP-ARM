...
bool CWin32File::Open(const CURL& url)
{
  ...
  if (m_hFile == INVALID_HANDLE_VALUE)
  {
    m_hFile = CreateFileUWP(pathnameW, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_EXISTING);
  }
#else
...
bool CWin32File::OpenForWrite(const CURL& url, bool bOverWrite /*= false*/)
{
  ...
  if (m_hFile == INVALID_HANDLE_VALUE)
  {
    CreateFileUWP(pathnameW, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
...
bool CWin32File::Delete(const CURL& url)
{
  ...
  if (pathnameW.empty())
   ...
   bool result = (DeleteFileW(pathnameW.c_str()) != 0);
  if (!result)
  {
    result = DeleteUWP(pathnameW);
  }
  if (m_smbFile)
    m_lastSMBFileErr = GetLastError(); // set real error state
...
bool CWin32File::Rename(const CURL& urlCurrentName, const CURL& urlNewName)
{
  ...
  if (newNameW.empty())
   ...
   bool result = (MoveFileExW(curNameW.c_str(), newNameW.c_str(), MOVEFILE_COPY_ALLOWED) != 0);
  if (!result)
  {
    result = RenameUWP(curNameW, newNameW);
  }
  if (m_smbFile)
    m_lastSMBFileErr = GetLastError(); // set real error state
...
#ifdef TARGET_WINDOWS_STORE
   HANDLE hFile = CreateFile2(pathnameW.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
    OPEN_EXISTING, NULL);
  if (hFile == INVALID_HANDLE_VALUE)
  {
    hFile = CreateFileUWP(pathnameW, GENERIC_READ | GENERIC_WRITE,

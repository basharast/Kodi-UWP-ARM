...
bool CWin32Directory::GetDirectory(const CURL& url, CFileItemList &items)
{
  ...
  {
    auto contents = GetFolderContents(basePath);
    if (contents.empty())
    {
    ...
    else
    {
      ...
      {
        CFileItemPtr pItem(new CFileItem(item.name));

        pItem->m_bIsFolder = ((item.attributes & FILE_ATTRIBUTE_DIRECTORY) != 0);
        if (pItem->m_bIsFolder)
          pItem->SetPath(pathWithSlash + item.name + '\\');
        else
          pItem->SetPath(pathWithSlash + item.name);

        if ((item.attributes & (FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM)) != 0 ||
            item.name.front() == '.') // mark files starting from dot as hidden
        ...
        fileTime.lowDateTime = item.lastWriteTime;
        fileTime.highDateTime = item.lastWriteTime;
...
bool CWin32Directory::Create(const CURL& url)
{
  ...
  if (!Create(nameW))
  {
    if (CreateDirectoryUWP(nameW, false))
    {
        return true;
    }
    else

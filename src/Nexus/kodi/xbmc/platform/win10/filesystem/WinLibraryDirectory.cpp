...
StorageFolder CWinLibraryDirectory::GetRootFolder(const CURL& url)
{
  ...
  try
  {
    ...
    if (lib == "pshared")
    {
      auto folder = GetItemByKey(L"public_shared");
      if (folder == nullptr)
      {
        CGUIDialogKaiToast::QueueNotification("Shared folder",
                                              "Setup from Settings->File manager");
      }
...
bool CWinLibraryDirectory::IsValid(const CURL& url)
{
  ...
  if (lib == "music" || lib == "video" || lib == "pictures" || lib == "photos" ||
      lib == "documents" || lib == "removable" || lib == "pshared")
    return true;
  else
    return false;
}

...
extern StorageFolder PickPublicSharedFolder();
...
void CStorageProvider::GetLocalDrives(VECSOURCES &localDrives)
{
  ...
  CMediaSource picker;
  picker.strPath = CSpecialProtocol::TranslatePath("win-lib://pshared");
  picker.strName = g_localizeStrings.Get(1012) + " (None)";
  auto current = GetItemByKey(L"public_shared");
  if (current != nullptr)
  {
    auto name = convert(current.Name());
    picker.strName = g_localizeStrings.Get(1012) + " (" + name + ")";
  }
  picker.m_ignore = true;
  picker.m_iDriveType = CMediaSource::SOURCE_TYPE_LOCAL;
  localDrives.push_back(picker);

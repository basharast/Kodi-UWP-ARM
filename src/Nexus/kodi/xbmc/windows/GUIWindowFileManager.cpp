...
extern void PickPublicSharedFolder(
    CGUIWindowFileManager* instance, int iList);
extern IStorageItem GetItemByKey(winrt::hstring key);
...
bool CGUIWindowFileManager::Update(int iList, const std::string &strDirectory)
{
  ...
  { // add 'add source button'
    ...
    auto current = GetItemByKey(L"public_shared");
    if (current != nullptr)
    {
      auto name = convert(current.Name());
      const std::string& strSharedLabel2 = g_localizeStrings.Get(1012) + " (" + name + ")";
      CFileItemPtr pShared(new CFileItem(strSharedLabel2));
      pShared->SetPath("pshared");
      pShared->SetArt("icon", "DefaultAddSource.png");
      pShared->SetLabel(strSharedLabel2);
      pShared->SetLabelPreformatted(true);
      pShared->m_bIsFolder = true;
      pShared->SetSpecialSort(SortSpecialOnBottom);
      m_vecItems[iList]->Add(pShared);
    }
    else
    {
      const std::string& strSharedLabel = g_localizeStrings.Get(1012) + " (Setup)";
    CFileItemPtr pShared(new CFileItem(strSharedLabel));
    pShared->SetPath("pshared");
    pShared->SetArt("icon", "DefaultAddonVfs.png");
    pShared->SetLabel(strSharedLabel);
    pShared->SetLabelPreformatted(true);
    pShared->m_bIsFolder = true;
    pShared->SetSpecialSort(SortSpecialOnBottom);
    m_vecItems[iList]->Add(pShared);
...
void FinalizeSharedFolder(StorageFolder folder, CGUIWindowFileManager* instance, int iList)
{
  SetRenderGUI(true);
  if (folder != nullptr)
  {
    CGUIDialogKaiToast::QueueNotification(g_localizeStrings.Get(1012), "Shared folder selected");
    instance->Update(iList, "");
  }
}
...
void CGUIWindowFileManager::OnClick(int iList, int iItem)
{
  ...
  if (pItem->GetPath() == "pshared") // 'add source button' in empty root
  {
    std::string msg = "This folder will be visible in all sections";
    auto current = GetItemByKey(L"public_shared");
    if (current != nullptr)
    {
      msg.append(", current folder (" + convert(current.Path()) + ")");
      msg.append(", Do you want to change it?");
    }
    else
    {
      msg.append(", Do you want to choose one?");
    }
    if (CGUIDialogYesNo::ShowAndGetInput(CVariant{1012}, CVariant{msg}))
    {
      SetRenderGUI(false);
      PickPublicSharedFolder(this, iList);
    }
    return;
  }

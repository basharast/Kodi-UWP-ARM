...
public:
	...
	std::list<StorageFileW> GetFiles() {
		...
		CLog::LogF(LOGDEBUG, "Getting files for {}", GetPath());
		...
		CLog::LogF(LOGDEBUG, "Total files added in ({})", GetPath());
	...
	std::list<StorageFolderW> GetFolders() {
		...
		CLog::LogF(LOGDEBUG, "Getting folders for {}", GetPath());

		ExecuteTask(sFolders, storageFolder.GetFoldersAsync());
		if (sFolders != nullptr) {
			CLog::LogF(LOGDEBUG, "Sub folders founded ({})", GetPath());
		...
		CLog::LogF(LOGDEBUG, "Total folders added ({})", GetPath());
	...
	std::list<StorageFileW> GetAllFiles(bool useWindowsIndexer = false) {
			...
			CLog::LogF(LOGDEBUG, "Getting all files for {}", GetPath());
			IVectorView<StorageFile> sFiles;
	...
	std::list<StorageFolderW> GetAllFolders(bool useWindowsIndexer = false) {
		std::list<StorageFolderW> folders;
			...
			IVectorView<StorageFolder> sFolders;
			CLog::LogF(LOGDEBUG, "Getting all folders for {}", GetPath());
			...
			if (sFolders != nullptr) {
				CLog::LogF(LOGDEBUG, "Sub folders founded in ({})", GetPath());
				for (auto it = 0; it != sFolders.Size(); ++it) {
					auto sItem = sFolders.GetAt(it);
					if (sItem != nullptr) {
						folders.push_back(StorageFolderW(sItem));
			...
			CLog::LogF(LOGDEBUG, "Total folders added in ({})", GetPath());
	...
	bool Contains(PathUWP path, IStorageItem& storageItem) {
		...
		if (!path.IsAbsolute()) {
			CLog::LogF(LOGDEBUG, "Looking for ({})", pathString);

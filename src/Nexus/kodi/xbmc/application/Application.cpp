...
void CApplication::OnApplicationMessage(ThreadMessage* pMsg)
{
  ...
  switch (msg)
  {
    ...
    case TMSG_NETWORKMESSAGE:
      m_ServiceManager->GetNetwork().NetworkMessage(
...
int CApplication::Run()
{
  ...
  int fpslimits = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_VIDEOSCREEN_FPSLIMITS);
  static std::chrono::milliseconds frameTime(1000 / fpslimits);
  ...
  while (!m_bStop)
  {
    if (fpsValueChanged)
    {
      fpslimits = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_VIDEOSCREEN_FPSLIMITS);
      frameTime = std::chrono::milliseconds(1000 / fpslimits);
      fpsValueChanged = false;
    }

...
void CAESinkXAudio::EnumerateDevicesEx(AEDeviceInfoList& deviceInfoList, bool force)
{
  ...
  for (RendererDetail& details : CAESinkFactoryWin::GetRendererDetails())
  {
    ...
    CLog::LogF(LOGINFO, "Device default format %s.",
               details.smpwfxex != nullptr ? "detected" : "not found");
    CLog::LogF(LOGINFO, "------------------------");
...
bool CAESinkXAudio::InitializeInternal(std::string deviceId, AEAudioFormat& format)
{
  ...
  /*if (formatsList.size() > 0)
  {
    for (WAVEFORMATEXTENSIBLE* formatP : formatsList)
    {
      ...
      {
        CLog::LogF(LOGINFO, "Format is Supported - ignore others");
        defaulFormatFound = true;
        break;
      }
    }
  }*/

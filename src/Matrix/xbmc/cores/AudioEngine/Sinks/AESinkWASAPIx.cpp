...
bool CAESinkWASAPI::Initialize(AEAudioFormat& format, std::string& device)
{
  ...
  {
    CLog::LogF(LOGINFO, "Could not Initialize with that format");
    goto failed;
  }
...
//std::vector<WAVEFORMATEXTENSIBLE*> formatsList;
...
void CAESinkWASAPI::EnumerateDevicesEx(AEDeviceInfoList& deviceInfoList, bool force)
{
  ...
  for (RendererDetail& details : CAESinkFactoryWin::GetRendererDetails())
  {
    ...
    /*CLog::LogF(LOGINFO, "Selected device %s.", details.strDescription);
    CLog::LogF(LOGINFO, "Device default format %s.", details.smpwfxex != nullptr ? "detected" : "not found");
    CLog::LogF(LOGINFO, "------------------------");

    if (details.smpwfxex != nullptr)
    {
      CLog::Log(LOGDEBUG, "  Sample Rate     : %d", details.smpwfxex->Format.nSamplesPerSec);
      CLog::Log(LOGDEBUG, "  Bits Per Sample : %d", details.smpwfxex->Format.wBitsPerSample);
      CLog::Log(LOGDEBUG, "  Valid Bits/Samp : %d", details.smpwfxex->Samples.wValidBitsPerSample);
      CLog::Log(LOGDEBUG, "  Channel Count   : %d", details.smpwfxex->Format.nChannels);
      CLog::Log(LOGDEBUG, "  Block Align     : %d", details.smpwfxex->Format.nBlockAlign);
      CLog::Log(LOGDEBUG, "  Avg. Bytes Sec  : %d", details.smpwfxex->Format.nAvgBytesPerSec);
      CLog::Log(LOGDEBUG, "  Samples/Block   : %d", details.smpwfxex->Samples.wSamplesPerBlock);
      CLog::Log(LOGDEBUG, "  Format cBSize   : %d", details.smpwfxex->Format.cbSize);
      CLog::Log(LOGDEBUG, "  Channel Mask    : %d", details.smpwfxex->dwChannelMask);
      CLog::LogF(LOGINFO, "------------------------");

      formatsList.push_back(details.smpwfxex);
    }
    ...
    if (SUCCEEDED(hr))
    {
      ...
      hr = pClient->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, &wfxex.Format,
                                      (WAVEFORMATEX**)&closest);
      if (hr == AUDCLNT_E_EXCLUSIVE_MODE_NOT_ALLOWED || hr == AUDCLNT_E_UNSUPPORTED_FORMAT)
      {
        CLog::LogF(
            LOGWARNING,
            "DTS-HD-HR mode is not allowed or supported on device \"%s\", check device settings.",
      ...
      if (hr == AUDCLNT_E_EXCLUSIVE_MODE_NOT_ALLOWED || hr == AUDCLNT_E_UNSUPPORTED_FORMAT)
      {
        CLog::LogF(LOGWARNING,
                   "DTS-HD mode is not allowed on device \"%s\", check device settings.",
                   details.strDescription);
        //SafeRelease(&pDevice);
        //continue;
      }
      ...
      {
        CLog::LogF(LOGWARNING, "Dolby TrueHD on \"%s\" seems to be not supported.",
                   details.strDescription);
      }
      else if (SUCCEEDED(hr) || details.eDeviceType == AE_DEVTYPE_HDMI)
      {
        if (FAILED(hr))
        {
          CLog::LogF(LOGINFO, "stream type \"%s\" on device \"%s\" seems to be not supported.",
      ...
      if (hr == AUDCLNT_E_UNSUPPORTED_FORMAT)
      {
        CLog::LogF(LOGWARNING, "DTS on \"%s\" seems to be not supported.", details.strDescription);
      }
      else if (SUCCEEDED(hr) || details.eDeviceType == AE_DEVTYPE_HDMI)
      {
        if (FAILED(hr))
      ...
      if (hr == AUDCLNT_E_UNSUPPORTED_FORMAT)
      {
        CLog::LogF(LOGWARNING, "Dolby AC3 on \"%s\" seems to be not supported.",
                   details.strDescription);
      }
      else if (SUCCEEDED(hr) || details.eDeviceType == AE_DEVTYPE_HDMI)
      {
      ...
      for (int p = AE_FMT_FLOAT; p > AE_FMT_INVALID; p--)
      {
        ...
        if (hr == AUDCLNT_E_UNSUPPORTED_FORMAT)
        {
          CLog::LogF(LOGWARNING, "PCM format on \"%s\" seems to be not supported.",
                     details.strDescription);
        }
...
bool CAESinkWASAPI::InitializeExclusive(AEAudioFormat& format)
{
  ...
  {
    for (WAVEFORMATEXTENSIBLE* formatP : formatsList)
    {
      WAVEFORMATEX formatTest = formatP->Format;

      wfxex.Format.wFormatTag = formatTest.wFormatTag;
      wfxex.Format.nChannels = formatTest.nChannels;
      wfxex.Format.nSamplesPerSec = formatTest.nSamplesPerSec;
      wfxex.Format.wBitsPerSample = formatTest.wBitsPerSample;
      wfxex.Format.nBlockAlign = formatTest.nBlockAlign;
      wfxex.Samples.wValidBitsPerSample = formatP->Samples.wValidBitsPerSample;
      wfxex.Format.cbSize = sizeof(WAVEFORMATEXTENSIBLE) - sizeof(WAVEFORMATEX);
      wfxex.Format.nAvgBytesPerSec = wfxex.Format.nBlockAlign * wfxex.Format.nSamplesPerSec;
      wfxex.dwChannelMask = formatP->dwChannelMask;
      wfxex.SubFormat = formatP->SubFormat;

      hr = m_pAudioClient->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, &wfxex.Format,
                                             (WAVEFORMATEX**)&closest);
      CLog::LogF(LOGDEBUG, "IsFormatSupported (%s)", WASAPIErrToStr(hr));
      if (SUCCEEDED(hr) && hr != AUDCLNT_E_UNSUPPORTED_FORMAT)
      {
  ...
  */
  ...
  hr = m_pAudioClient->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, &wfxex.Format,
                                                 (WAVEFORMATEX**)&closest);
  CLog::LogF(LOGINFO, "IsFormatSupported (%s)", WASAPIErrToStr(hr));
  if (SUCCEEDED(hr) && hr != AUDCLNT_E_UNSUPPORTED_FORMAT)
  {
    CLog::LogF(LOGINFO, "Format is Supported - will attempt to Initialize");
    goto initialize;
  ...
  {
    CLog::LogF(LOGINFO, "Use closest format");
    useClosest = true;
    goto initialize;
  }
  ...
  for (int layout = -1; layout <= (int)ARRAYSIZE(layoutsList); layout++)
  {
    ...
    for (int j = 0; j < sizeof(testFormats) / sizeof(sampleFormat); j++)
    {
      ...
      for (int i = 0; i < WASAPISampleRateCount; i++)
      {
...
#if 1
        CLog::Log(LOGDEBUG, "WASAPI: Trying Format: %s, %d, %d, %d",
                  CAEUtil::DataFormatToStr(testFormats[j].subFormatType),
                  wfxex.Format.nSamplesPerSec, wfxex.Format.wBitsPerSample,
        ...
        {
          CLog::LogF(LOGERROR, "IsFormatSupported (%s)", WASAPIErrToStr(hr));
        }
      }
...
initialize:
  ...
  hr = m_pAudioClient->Initialize(AUDCLNT_SHAREMODE_SHARED,
                                  ...
                                  useClosest ? &closest->Format : &wfxex.Format, NULL);
  ...
  if (FAILED(hr))
  {
    CLog::LogF(LOGERROR, "Failed to initialize WASAPI in %d - (%s).", HRESULT(hr),
               WASAPIErrToStr(hr));
  ...
  CLog::LogF(LOGINFO, "WASAPI Sink Initialized using: %s, %d, %d",
